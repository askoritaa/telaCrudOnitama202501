...
class SkinModel {
  ...
  list(limit, offset) {
    const sql = "SELECT * FROM conjuntos_skins WHERE active = true ORDER BY created_at DESC LIMIT ? OFFSET ?";
    return this.executeQuery(sql, [limit, offset]);
  }

  countAll() {
    const sql = "SELECT COUNT(*) AS total_rows FROM conjuntos_skins WHERE active = true;";
